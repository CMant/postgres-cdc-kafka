...
rd_kafka_t *copy_g_rk = NULL;
rd_kafka_topic_t *copy_g_rkt = NULL;
// rd_kafka_conf_t *copy_g_conf = NULL;
 
volatile int kafka_initialized = 0; 
...
extern CopyJsonInfo g_json_copy_info;
// extern rd_kafka_topic_t *copy_message_rkt;
// extern rd_kafka_t *copy_message_rk;
...
bool CopyFromTextOneRow(CopyFromState cstate, ExprContext *econtext, Datum *values,
						bool *nulls)
{
...
bool CopyFromCSVOneRow(CopyFromState cstate, ExprContext *econtext, Datum *values,
					   bool *nulls)
{
...
#if 1
// 获得数据库的名称。根据订阅获得
// get_dbname_from_conninfo(MySubscription->conninfo,&g_json_copy_info.dbname,strlen(MySubscription->conninfo));

static void COPY_TO_JSON(CopyJsonInfo *g_json_copy_info)
{
     //

	char *json_str = NULL;
	cJSON *root = NULL;
	cJSON *array_attnames = NULL;
	cJSON *array_after = NULL;
    if (!copy_g_rk && kafka_initialized == 0)
	{
		if (kafka_initialized == 0)
		{ // 双重检查
			if (kafka_init(&copy_g_rk, &copy_g_rkt, g_json_copy_info->kafka_server, g_json_copy_info->dbname) == 0)
			{
	...
	cJSON_AddStringToObject(root, "op", "C");
	cJSON_AddStringToObject(root, "db",
							g_json_copy_info->dbname ? g_json_copy_info->dbname : "");
	cJSON_AddStringToObject(root, "table", g_json_copy_info->table_name);
	...
	for (int i = 0; i < g_json_copy_info->field_count; i++)
	{

		if (g_json_copy_info->attnames[i])
			cJSON_AddItemToArray(array_attnames, cJSON_CreateString(g_json_copy_info->attnames[i]));

		if (g_json_copy_info->after[i] == NULL || strlen(g_json_copy_info->after[i]) == 0)
			cJSON_AddItemToArray(array_after, cJSON_CreateNull());
		else
			cJSON_AddItemToArray(array_after, cJSON_CreateString(g_json_copy_info->after[i]));
	}
	...
	if (json_str)
	{
		...
		kafka_send_optimized(json_str, copy_g_rk, copy_g_rkt);
		pfree(json_str);
	}
	cJSON_Delete(root);
}
					   ...
					   Datum *values, bool *nulls, bool is_csv)
{
	...
	//  snprintf(g_json_copy_info.table_name, sizeof(g_json_copy_info.table_name),
	//          "%s.%s",
	//          get_namespace_name(cstate->rel->rd_rel->relnamespace),
	//          cstate->cur_relname);
	// snprintf(g_json_copy_info.dbname, sizeof(g_json_copy_info.dbname),
	//          "%s",
	//          get_database_name(MyDatabaseId));

	// dbname  schema.table_name   字段名称和 字段数量，都从tablesync.c copyform中获取。
    /*
	copy的话，这两个加不了。这也代码调用的时候，initdb会使用，copy命令会使用。但是订阅参数需要从别的地方传过来
	...
	*/ 
	...
	g_json_copy_info.after = (char **)palloc0(cstate->rel->rd_att->natts * sizeof(char *));
	...
	foreach (cur, cstate->attnumlist)
	{
		...
		if (is_csv)
		{
			...
			else if (string != NULL && cstate->opts.force_null_flags[m] && strcmp(string, cstate->opts.null_print) == 0)
			{
				/*
		...
		g_json_copy_info.after[cur_val] = pstrdup(cstate->cur_attval);

        /*
			elog(LOG,"copy_file %s\n",cstate->copy_file); 
			如果是initdb初始化的，这里copy_file  不是空的
     ...
     if (g_json_copy_info.is_not_initdb) {
			COPY_TO_JSON(&g_json_copy_info);
		
	 }
	 
	// pfree(g_json_copy_info.attnames);
	pfree(g_json_copy_info.after);
...
bool CopyFromBinaryOneRow(CopyFromState cstate, ExprContext *econtext, Datum *values,
						  bool *nulls)
{
	TupleDesc tupDesc;
	AttrNumber attr_count;
	...
	for (;;)
	{
				...
				cstate->eol_type == EOL_CRNL)
			{
				...
				else
				{
					...
					if (cstate->eol_type == EOL_CRNL)
						ereport(ERROR,
								(errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
								 !is_csv ? errmsg("literal carriage return found in data") : errmsg("unquoted carriage return found in data"),
								 !is_csv ? errhint("Use \"\\r\" to represent carriage return.") : errhint("Use quoted CSV field to represent carriage return.")));
		...
		if (c == '\n' && (!is_csv || !in_quote))
		{
			if (cstate->eol_type == EOL_CR || cstate->eol_type == EOL_CRNL)
				ereport(ERROR,
						(errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
						 !is_csv ? errmsg("literal newline found in data") : errmsg("unquoted newline found in data"),
