...
#include "kafka.h"
#include "utils/guc.h"
CopyJsonInfo g_json_copy_info;
// extern char apply_dbname[64];
extern rd_kafka_t *copy_g_rk;
extern rd_kafka_topic_t *copy_g_rkt;
extern volatile int kafka_initialized;
	...
	while (maxread > 0 && bytesread < minread)
	{
		...
		for (;;)
		{
			...
			else
			{
				...
				// COPY_TO_JSON(copybuf->data);
				/*(gdb) p &copybuf->data[copybuf->cursor]
	  $50 = 0x1210100 "208\tddd\t2026-05-09 17:03:54.279995\n"
	  (gdb) p outbuf
	  $51 = (void *) 0x12f4b48
	  (gdb) p (char *)outbuf
	  $52 = 0x12f4b48 "207\tddd\t2026-05-09 17:03:54.279995\n"
	  (gdb) p copybuf->data
	  $53 = 0x1210100 "208\tddd\t2026-05-09 17:03:54.279995\n"
	  (gdb) p copybuf->len*/
				avail = copybuf->len - copybuf->cursor;
				if (avail > maxread)
					avail = maxread;
						...
						List **qual, bool *gencol_published)
{
	...
	appendStringInfo(&cmd,
					 ...
					 " ORDER BY a.attnum",
					 lrel->remoteid,
					 (server_version >= 120000 && server_version < 180000 ? "AND a.attgenerated = ''" : ""),
					 lrel->remoteid);
	res = walrcv_exec(LogRepWorkerWalRcvConn, cmd.data,
...
/*
 * 全量同步 copy_data 时：每解析一行数据，就会调用这里！
 * 你在这里拿数据、生成 JSON、发 Kafka
 */
...
copy_table(Relation rel)
{
	...
	// 获取源端数据库名称函数，copyfromparse.c通过extern g_json_copy_info 获取。
	get_dbname_from_conninfo(MySubscription->conninfo, g_json_copy_info.dbname, strlen(MySubscription->conninfo));
	// 这里启动rdkafa

	strncpy(g_json_copy_info.kafka_server, GetConfigOption("kafka_server", true, false), strlen(GetConfigOption("kafka_server", true, false)));
	// kafka_init_copy(g_json_copy_info.kafka_server, g_json_copy_info.dbname);
	g_json_copy_info.is_not_initdb = 1;
	...
	else
	{
		...
		for (int i = 0; i < lrel.natts; i++)
		{
			...
			if (i < lrel.natts - 1)
				appendStringInfoString(&cmd, ", ");
			// 这里获得字段列表，cmd 会拼凑成 	COPY (SELECT id, val, create_time" 具体字段信息来自lrel.attnames[i]
		}
		// 这里获取schema 和 tablename
	...
	snprintf(g_json_copy_info.table_name, 128, "%s.%s", lrel.nspname, lrel.relname);
	g_json_copy_info.field_count = lrel.natts;
	g_json_copy_info.attnames = (char **)palloc0(g_json_copy_info.field_count * sizeof(char *));
	for (int g = 0; g < g_json_copy_info.field_count; g++)
	{
		g_json_copy_info.attnames[g] = pstrdup(lrel.attnames[g]);
	}
	/*
	...
	// cstate = BeginCopyFrom_NOCOPY(pstate, rel, NULL, NULL, false, copy_read_data, attnamelist, options);
	...
	//kafka_close_copy(&copy_g_rk, &copy_g_rkt,g_json_copy_info.table_name);  不能在这里使用close，程序会崩溃
...
copy_table_done:
	...
	kafka_close_copy(&copy_g_rk, &copy_g_rkt,rel->rd_rel->relname.data);
	return slotname;
}
