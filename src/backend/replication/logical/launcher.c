...
ApplyLauncherMain(Datum main_arg)
{
	...
	for (;;)
	{
		...
		foreach(lc, sublist)
		{
			...
			/*p *sub
$11 = {oid = 65601, dbid = 5, skiplsn = 0, name = 0x1966e10 "vmx_test_table3", owner = 10, ownersuperuser = false, enabled = true, binary = false, stream = 0 '\000', twophasestate = 0 '\000', 
  disableonerr = false, passwordrequired = false, runasowner = false, failover = false, conninfo = 0x0, slotname = 0x0, synccommit = 0x0, publications = 0x0, origin = 0x0}
   这里初次获得订阅信息。可以在这里创建kafka
