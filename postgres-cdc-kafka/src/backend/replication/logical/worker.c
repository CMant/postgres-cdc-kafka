...
#if 1
static char apply_dbname[64];
 
static cJSON *array_after;
static cJSON *array_before;
static cJSON *array_attnames;
static cJSON *root;
...
// static rd_kafka_conf_t *apply_g_conf = NULL;
static volatile int kafka_initialized = 0; // 使用volatile替代atomic_int
// static pthread_mutex_t kafka_init_mutex = PTHREAD_MUTEX_INITIALIZER;

#endif

/* 缓冲区总大小 */
// static size_t g_my_buffer_size = 0;
 ...
 */
void ReplicationOriginNameForLogicalRep(Oid suboid, Oid relid,
 ...
 */
void apply_spooled_messages(FileSet *stream_fileset, TransactionId xid,
							XLogRecPtr lsn)
{
	int nchanges;
	char path[MAXPGPATH];
...
static void check_kafka_init(){
	if (!apply_g_rk && kafka_initialized == 0)
	{
		if (kafka_initialized == 0)
		{ // 双重检查
			get_dbname_from_conninfo(MySubscription->conninfo, apply_dbname, strlen(MySubscription->conninfo));
			kafka_server = (char *)GetConfigOption("kafka_server", true, false);
			if (kafka_init(&apply_g_rk, &apply_g_rkt, kafka_server, apply_dbname) == 0)
			{
				kafka_initialized = 1; // 直接赋值替代atomic_store
			}
			else
			{

				ereport(ERROR, (errhidecontext(true), errmsg("Kafka未正确初始化")));
...
#if 1
static void INSERT_TO_JSON(LogicalRepRelMapEntry *rel, LogicalRepTupleData *newtup)
{
	...
	if (!root)
	{
		...
		ereport(ERROR, (errhidecontext(true), errmsg("cJSON 创建对象失败")));
		return;
	}

	cJSON_AddStringToObject(root, "op", "I");
	cJSON_AddStringToObject(root, "db", apply_dbname);
	snprintf(table_name, 128, "%s.%s", rel->remoterel.nspname, rel->remoterel.relname);
	cJSON_AddStringToObject(root, "table", table_name);

	array_attnames = cJSON_CreateArray();
	cJSON_AddItemToObject(root, "attnames", array_attnames);

	array_after = cJSON_CreateArray();
	cJSON_AddItemToObject(root, "after", array_after);

	for (int i = 0; i < newtup->ncols; i++)
	{
		cJSON_AddItemToArray(array_attnames, cJSON_CreateString(rel->remoterel.attnames[i]));

		if (newtup->colvalues[i].data == NULL)
		{
			cJSON_AddItemToArray(array_after, cJSON_CreateNull());
		}
		else
		{
			cJSON_AddItemToArray(array_after, cJSON_CreateString(newtup->colvalues[i].data));
		}
	}
	...
	if (json_str)
	{
		...
		kafka_send_optimized(json_str, apply_g_rk, apply_g_rkt);
		pfree(json_str);
	}
...
apply_handle_insert(StringInfo s)
{
	...
	// MemoryContext oldctx;
	// bool		run_as_owner;
	// ereport(LOG, (errmsg("=== 我的调试：relid = %u, 表名 = %s", relid, rel->relname)));
	/*
	 * Quick return if we are skipping data modification changes or handling
	 * streamed transactions.
	 */
	// pg_usleep(30 * 1000000);
	if (is_skipping_changes() ||
	...
	/* 这里newtup已经有了完整的数据
	  (gdb) print rel->remoterel
$6 = {remoteid = 27688896, nspname = 0x4f <error: Cannot access memory at address 0x4f>,
  relname = 0x145c0000004f <error: Cannot access memory at address 0x145c0000004f>, natts = 27674128, attnames = 0x1a67fc0,
  atttyps = 0xffffffff8bd3ab06, replident = 0 '\000', relkind = 0 '\000', attkeys = 0x9a6886 <apply_error_callback>}
(gdb) print newtup->colvalues
$7 = (StringInfoData *) 0x1b17fb0
(gdb) p (char*)newtup->colvalues[0]
$8 = 0x1b18058 "73"
(gdb) p (char*)newtup->colvalues[1]
$9 = 0x1b18070 "ddd"
(gdb) p (char*)newtup->colvalues[2]
$10 = 0x1b18088 "2026-05-09 11:12:17.618091"
(gdb)
	做cdc工具的话。需要
	I: INSERT  表示操作
	I TRX_ID COMMIT_LSN TABLE_NAME  COL_NAME   VALUE
	之前是RowExclusiveLock 。 反正也不需要写入数据，改成了AS锁。但是不能使用NoLock。底层校验不通过。logicalrep_rel_open () 不允许传 NoLock

	如果想打印上下文elog(NOTICE  换成  elog(log
	*/

	// rel = logicalrep_rel_open(relid, RowExclusiveLock);
	rel = logicalrep_rel_open(relid, AccessShareLock);

	INSERT_TO_JSON(rel, &newtup);
...
static void UPDATE_TO_JSON(LogicalRepRelMapEntry *rel, bool *has_oldtup, LogicalRepTupleData *oldtup, LogicalRepTupleData *newtup)
{
    // cJSON *array_before;
	// cJSON *array_after;
	// cJSON *array_attnames;
	// char table_name[128];

	// cJSON *root; // JSON根对象
	// // cJSON *array;      // 数组对象
	// char *json_str; // 最终生成的JSON字符串
	...
	if (!root)
	{
		elog(ERROR, "cJSON 创建对象失败");
		ereport(ERROR, (errhidecontext(true), errmsg("cJSON 创建对象失败")));
		return;
	}

	// lsn_str2 = psprintf("%08X/%08X",
	//                  (uint32) (apply_error_callback_arg.finish_lsn >> 32),  // 高32位
	//                  (uint32) apply_error_callback_arg.finish_lsn);         // 低32位
	// lsn_str2 = pg_lsn_to_string(apply_error_callback_arg.finish_lsn);
	cJSON_AddStringToObject(root, "op", "U"); // 字符串

	// elog(LOG,"MySubscription->conninfo:%s\n",MySubscription->conninfo);
	cJSON_AddStringToObject(root, "db", apply_dbname);
	// cJSON_AddStringToObject(root, "lsn", lsn_str2);    // 字符串
	snprintf(table_name, 128, "%s.%s", rel->remoterel.nspname, rel->remoterel.relname);
	// cJSON_AddStringToObject(root, "schema", rel->remoterel.nspname);    // 字符串
	// cJSON_AddStringToObject(root, "relname",  rel->remoterel.relname);    // 字符串
	cJSON_AddStringToObject(root, "table", table_name);

	array_attnames = cJSON_CreateArray();
	cJSON_AddItemToObject(root, "attnames", array_attnames);

	array_before = cJSON_CreateArray();
	cJSON_AddItemToObject(root, "before", array_before);
	...
	if (*has_oldtup)
	{
		...
		{
			if (oldtup->colvalues[i].data == NULL)
			{
				cJSON_AddItemToArray(array_before, cJSON_CreateNull());
			}
			else
			{
				cJSON_AddItemToArray(array_before, cJSON_CreateString(oldtup->colvalues[i].data));
			}
		}
	}
...
apply_handle_update(StringInfo s)
{
	...
	UPDATE_TO_JSON(rel, &has_oldtup, &oldtup, &newtup);
#if 0		
...
static void DELETE_TO_JSON(LogicalRepRelMapEntry *rel, LogicalRepTupleData *oldtup)
{
	...
	//                  (uint32) apply_error_callback_arg.finish_lsn);         // 低32位
	// lsn_str2 = pg_lsn_to_string(apply_error_callback_arg.finish_lsn);
	cJSON_AddStringToObject(root, "op", "D"); // 字符串
	// get_dbname_from_conninfo(MySubscription->conninfo,apply_dbname, 64);
	cJSON_AddStringToObject(root, "db", apply_dbname);
	// cJSON_AddStringToObject(root, "lsn", lsn_str2);    // 字符串
	snprintf(table_name, 128, "%s.%s", rel->remoterel.nspname, rel->remoterel.relname);
	// cJSON_AddStringToObject(root, "schema", rel->remoterel.nspname);    // 字符串
...
apply_handle_truncate(StringInfo s)
{
	 ...
	 */
	// 这部分真正执行truncate。其他都是校验，实际上都注释掉也可以。实际上truncate 不屏蔽也没事儿。本身也没数据
...
LogicalRepApplyLoop(XLogRecPtr last_received)
{
	...
	// // 1. 创建配置
	// 	apply_g_conf = rd_kafka_conf_new();
	// 	if (rd_kafka_conf_set(apply_g_conf, "bootstrap.servers", kafka_server,
	// 							apply_g_errstr, sizeof(apply_g_errstr)) != RD_KAFKA_CONF_OK) {
	// 		elog(LOG, "conf set fail: %s\n", apply_g_errstr);
	// 		return;
	// 	}

	// 	// 2. 创建生产者实例
	// 	apply_g_rk = rd_kafka_new(RD_KAFKA_PRODUCER, apply_g_conf, apply_g_errstr, sizeof(apply_g_errstr));
	// 	if (!apply_g_rk) {
	// 		elog(LOG, "new producer fail: %s\n", apply_g_errstr);
	// 		return;
	// 	}

	// 	// 3. 创建 topic
	// 	apply_g_rkt = rd_kafka_topic_new(apply_g_rk, apply_dbname, NULL);

	//  kafka_init_apply(apply_message_rk,apply_message_rkt,apply_message_conf,kafka_server,apply_dbname);
	...
	// child_process_work(apply_dbname);
						...
						XLogRecPtr *origin_startpos)
{
	...
	options->proto.logical.proto_version =
		server_version >= 160000 ? LOGICALREP_PROTO_STREAM_PARALLEL_VERSION_NUM : server_version >= 150000 ? LOGICALREP_PROTO_TWOPHASE_VERSION_NUM
...
run_apply_worker()
{
	...
	ereport(DEBUG1,
			(errmsg_internal("logical replication apply worker for subscription \"%s\" two_phase is %s",
							 MySubscription->name,
							 MySubscription->twophasestate == LOGICALREP_TWOPHASE_STATE_DISABLED ? "DISABLED" : MySubscription->twophasestate == LOGICALREP_TWOPHASE_STATE_PENDING ? "PENDING"
...
void ApplyWorkerMain(Datum main_arg)
{
	...
	// char apply_message_errstr[256];
	//  	if(!apply_message_rkt&&!apply_message_rk){
	//  	get_dbname_from_conninfo(MySubscription->conninfo,apply_dbname,strlen(MySubscription->conninfo));
	//   kafka_server = GetConfigOption("kafka_server", true, false);
	//  //#################启动kafka

	// // 1. 创建配置
	// 	apply_message_conf = rd_kafka_conf_new();
	// 	if (rd_kafka_conf_set(apply_message_conf, "bootstrap.servers", kafka_server,
	// 							apply_message_errstr, sizeof(apply_message_errstr)) != RD_KAFKA_CONF_OK) {
	// 		elog(LOG, "conf set fail: %s\n", apply_message_errstr);
	// 		return;
	// 	}

	// 	// 2. 创建生产者实例
	// 	apply_message_rk = rd_kafka_new(RD_KAFKA_PRODUCER, apply_message_conf, apply_message_errstr, sizeof(apply_message_errstr));
	// 	if (!apply_message_rk) {
	// 		elog(LOG, "new producer fail: %s\n", apply_message_errstr);
	// 		return;
	// 	}
	...
	// 	// 3. 创建 topic
	// 	apply_message_rkt = rd_kafka_topic_new(apply_message_rk, apply_dbname, NULL);
