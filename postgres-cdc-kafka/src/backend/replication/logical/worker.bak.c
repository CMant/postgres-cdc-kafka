...
#if 1
char apply_dbname[64];

// rd_kafka_conf_t *apply_message_conf;

#endif

static rd_kafka_topic_t *apply_g_rkt;
static rd_kafka_t        *apply_g_rk;
static rd_kafka_conf_t *apply_g_conf;
/* 缓冲区总大小 */
...
static void INSERT_TO_JSON(LogicalRepRelMapEntry *rel,LogicalRepTupleData * newtup)
{
    ...
    get_dbname_from_conninfo(MySubscription->conninfo, dbname, 64);
    cJSON_AddStringToObject(root, "db", dbname);
    ...
    if (json_str) {
        topic_kafka_send(dbname, json_str);
        pfree(json_str);
    }
 ...
 */
 ...
 static void UPDATE_TO_JSON(LogicalRepRelMapEntry *rel,bool	*has_oldtup,LogicalRepTupleData *oldtup,LogicalRepTupleData * newtup){
     //需要转义吗？如果转义，那就需要pg，mysql 两种方式进行转义
	//char lsn_str[1024];
	  //  char *lsn_str2;
	    cJSON *array_before;
		cJSON *array_after;
	...
	//char lsn_str[1024];
		...
		for (int i = 0; i < oldtup->ncols; i++)
		 ...
		 if (json_str) {
			...
			//elog(LOG, "生成的JSON：\n%s", json_str);
			topic_kafka_send(apply_dbname,json_str);
			//kafka_send_apply(apply_message_rk,apply_message_rkt,json_str);
