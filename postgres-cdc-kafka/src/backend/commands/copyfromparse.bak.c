...
extern CopyJsonInfo g_json_info;
...
#if 1
// 获得数据库的名称。根据订阅获得
// get_dbname_from_conninfo(MySubscription->conninfo,&g_json_info.dbname,strlen(MySubscription->conninfo));

static void COPY_TO_JSON(CopyJsonInfo *g_json_info)
{
     //
    if (!copy_g_rk && kafka_initialized == 0)
	{
		...
		{ // 双重检查
			if (kafka_init(&copy_g_rk, &copy_g_rkt, g_json_info->kafka_server, g_json_info->dbname) == 0)
			{
	...
	cJSON_AddStringToObject(root, "db",
							g_json_info->dbname ? g_json_info->dbname : "");
	cJSON_AddStringToObject(root, "table", g_json_info->table_name);
	...
	for (int i = 0; i < g_json_info->field_count; i++)
	{

		if (g_json_info->attnames[i])
			cJSON_AddItemToArray(array_attnames, cJSON_CreateString(g_json_info->attnames[i]));

		if (g_json_info->after[i] == NULL || strlen(g_json_info->after[i]) == 0)
			cJSON_AddItemToArray(array_after, cJSON_CreateNull());
		else
			cJSON_AddItemToArray(array_after, cJSON_CreateString(g_json_info->after[i]));
	}
					   ...
					   Datum *values, bool *nulls, bool is_csv)
{
	...
	//  snprintf(g_json_info.table_name, sizeof(g_json_info.table_name),
	//          "%s.%s",
	//          get_namespace_name(cstate->rel->rd_rel->relnamespace),
	//          cstate->cur_relname);
	// snprintf(g_json_info.dbname, sizeof(g_json_info.dbname),
	//          "%s",
	...
	*/ 
	...
	g_json_info.after = (char **)palloc0(cstate->rel->rd_att->natts * sizeof(char *));
	...
	foreach (cur, cstate->attnumlist)
	{
		...
		g_json_info.after[cur_val] = pstrdup(cstate->cur_attval);
     ...
     if (g_json_info.is_not_initdb) {
			COPY_TO_JSON(&g_json_info);
		
	 }
	 
	// pfree(g_json_info.attnames);
	pfree(g_json_info.after);
