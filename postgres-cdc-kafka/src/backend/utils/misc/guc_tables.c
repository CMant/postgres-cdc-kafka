...
// --- 整型参数 ---
int kafka_queue_buffering_max_messages;
int kafka_queue_buffering_max_kbytes;
int kafka_queue_enqueue_timeout_ms;
int kafka_batch_num_messages;
int kafka_retries;
int kafka_retry_backoff_ms;
int kafka_message_timeout_ms;
int kafka_socket_send_buffer_bytes;
int kafka_socket_receive_buffer_bytes;
int kafka_socket_connection_setup_timeout_ms;
int kafka_metadata_max_age_ms;
int kafka_compression_level;
int kafka_log_level;

// --- 布尔类型参数 ---
bool kafka_enable_ssl_certificate_verification;
bool kafka_enable_idempotence;
bool kafka_enable_gapless_guarantee;
bool kafka_log_connection_close;
...
struct config_bool ConfigureNamesBool[] =
	{
		...
		{{"kafka_enable_ssl_certificate_verification",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Enable OpenSSL's builtin broker (server) certificate verification. This verification can be extended by the application by implementing a certificate_verify_cb. Type: boolean"),
		  gettext_noop("Default: true")},
		 &kafka_enable_ssl_certificate_verification,
		 true,
		 NULL,
		 NULL,
...
struct config_int ConfigureNamesInt[] =
	{
		...
		{{"kafka_queue_buffering_max_messages",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Maximum number of messages allowed on the producer queue. This queue is shared by all topics and partitions. A value of 0 disables this limit. Type: integer"),
		  gettext_noop("Default: 1000, Range: 1..2147483647")},
		 &kafka_queue_buffering_max_messages,
		 1000,   // 默认值
		 1,		   // 最小值
		 2147483647, // 最大值
		...
		{{"kafka_queue_buffering_max_kbytes",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Maximum total message size sum allowed on the producer queue. This queue is shared by all topics and partitions. This property has higher priority than queue.buffering.max.messages. Type: integer"),
		  gettext_noop("Default: 1048576 (1GB), Range: 1..2147483647")},
		 &kafka_queue_buffering_max_kbytes,
		 1048576,	 // 默认值 1GB
		 1,			 // 最小值
		 2147483647, // 最大值 1GB
		...
		{{"kafka_batch_num_messages",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Maximum number of messages batched in one MessageSet. The total MessageSet size is also limited by batch.size and message.max.bytes. Type: integer"),
		  gettext_noop("Default: 10000, Range: 1..1000000")},
		 &kafka_batch_num_messages,
		 10000,	  // 默认值
		 1,		  // 最小值
		 1000000, // 最大值
		...
		{{"kafka_retries",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Alias for message.send.max.retries: How many times to retry sending a failing Message. Note: retrying may cause reordering unless enable.idempotence is set to true. Type: integer"),
		  gettext_noop("Default: 2147483647, Range: 0..2147483647")},
		 &kafka_retries,
		 2147483647, // 默认值 无限重试
		 0,			 // 最小值
		 2147483647, // 最大值
		...
		{{"kafka_retry_backoff_ms",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("The backoff time in milliseconds before retrying a protocol request, this is the first backoff time, and will be backed off exponentially until number of retries is exhausted, and it's capped by retry.backoff.max.ms. Type: integer"),
		  gettext_noop("Default: 100ms, Range: 1..300000")},
		 &kafka_retry_backoff_ms,
		 100,	 // 默认值 100ms
		 1,		 // 最小值
		 300000, // 最大值 5分钟
		...
		{{"kafka_message_timeout_ms",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Local message timeout. This value is only enforced locally and limits the time a produced message waits for successful delivery. A time of 0 is infinite. This is the maximum time librdkafka may use to deliver a message (including retries). Delivery error occurs when either the retry count or the message timeout are exceeded. The message timeout is automatically adjusted to transaction.timeout.ms if transactional.id is configured. Type: integer"),
		  gettext_noop("Default: 300000ms (5min), Range: 0..2147483647")},
		 &kafka_message_timeout_ms,
		 300000, // 默认值 5分钟
		 0,		 // 最小值
		 2147483647, // 最大值 15分钟
		...
		{{"kafka_socket_send_buffer_bytes",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Broker socket send buffer size. System default is used if 0. Type: integer"),
		  gettext_noop("Default: 0 (use system default), Range: 0..100000000")},
		 &kafka_socket_send_buffer_bytes,
		 0,			// 默认值 系统默认
		 0,			// 最小值
		 100000000, // 最大值 100MB
		...
		{{"kafka_socket_receive_buffer_bytes",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Broker socket receive buffer size. System default is used if 0. Type: integer"),
		  gettext_noop("Default: 0 (use system default), Range: 0..100000000")},
		 &kafka_socket_receive_buffer_bytes,
		 0,			// 默认值 系统默认
		 0,			// 最小值
		...
		{{"kafka_socket_connection_setup_timeout_ms",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Maximum time allowed for broker connection setup (TCP connection setup as well SSL and SASL handshake). If the connection to the broker is not fully functional after this the connection will be closed and retried. Type: integer"),
		  gettext_noop("Default: 30000ms, Range: 1000..2147483647")},
		 &kafka_socket_connection_setup_timeout_ms,
		 30000, // 默认值 30秒
		 1000,	// 最小值
		 2147483647, // 最大值 60秒
		...
		{{"kafka_metadata_max_age_ms",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Metadata cache max age. Defaults to topic.metadata.refresh.interval.ms * 3.  Type: integer"),
		  gettext_noop("Default: 900000ms (15min), Range: 1..86400000")},
		 &kafka_metadata_max_age_ms,
		 900000,   // 默认值 15分钟
		 1,	   // 最小值 1秒
		 86400000, // 最大值 24小时
		...
		{{"kafka_compression_level",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Compression level parameter for algorithm selected by configuration property compression.codec. Higher values will result in better compression at the cost of more CPU usage. Usable range is algorithm-dependent: [0-9] for gzip; [0-12] for lz4; only 0 for snappy; -1 = codec-dependent default compression level. Type: integer"),
		  gettext_noop("Default: -1 (use library default), Range: -1..12")},
		 &kafka_compression_level,
		 -1, // 默认值 使用库默认
		 -1, // 最小值
		 12, // 最大值
		...
		{{"kafka_log_level",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Logging level (syslog(3) levels). Type: integer"),
		  gettext_noop("Default: 6 (LOG_INFO), Range: 0..7")},
		 &kafka_log_level,
		 6, // 默认值 LOG_INFO
		 0, // 最小值
		 7, // 最大值
		...
		{{"bgwriter_lru_maxpages", PGC_SIGHUP, RESOURCES_BGWRITER,
		 ...
		 100,
		 0,
		 INT_MAX / 2, /* Same upper limit as shared_buffers */
		 NULL,
		 NULL,
		 NULL},
...
struct config_string ConfigureNamesString[] =
	{
		...
		// kafka char 参数 start
		{
			{"kafka_server", PGC_SIGHUP, CUSTOM_OPTIONS,
			 gettext_noop("kafka server ip ,default values is 127.0.0.1:9092"),
			 NULL},
			&kafka_server,
			"127.0.0.1:9092",
			NULL,
		...
		{{"kafka_security_protocol",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Protocol used to communicate with brokers. Type: enum value"),
		  gettext_noop("Default: PLAINTEXT, Valid values: PLAINTEXT, SSL, SASL_PLAINTEXT, SASL_SSL")},
		 &kafka_security_protocol,
		 "PLAINTEXT",
		 NULL,
		 NULL,
		...
		{{"kafka_ssl_cipher_suites",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("A cipher suite is a named combination of authentication, encryption, MAC and key exchange algorithm used to negotiate the security settings for a network connection using TLS or SSL network protocol. See manual page for ciphers(1) and SSL_CTX_set_cipher_list(3). Type: string"),
		  gettext_noop("Default: NULL (use system defaults)")},
		 &kafka_ssl_cipher_suites,
		 NULL,
		 NULL,
		 NULL,
		...
		{{"kafka_ssl_key_location",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Path to client's private key (PEM) used for authentication. Type: string"),
		  gettext_noop("Default: NULL")},
		 &kafka_ssl_key_location,
		 NULL,
		 NULL,
		 NULL,
		...
		{{"kafka_ssl_key_password",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Private key passphrase (for use with ssl.key.location and set_ssl_cert()). Type: string"),
		  gettext_noop("Default: NULL")},
		 &kafka_ssl_key_password,
		 NULL,
		 NULL,
		...
		{{"kafka_ssl_certificate_location",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Path to client's public key (PEM) used for authentication. Type: string"),
		  gettext_noop("Default: NULL")},
		 &kafka_ssl_certificate_location,
		 NULL,
		 NULL,
		...
		{{"kafka_ssl_endpoint_identification_algorithm",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Endpoint identification algorithm to validate broker hostname using broker certificate. https - Server (broker) hostname verification as specified in RFC2818. none - No endpoint verification. OpenSSL >= 1.0.2 required. Type: enum value"),
		  gettext_noop("Default: none, Valid values: none, https")},
		 &kafka_ssl_endpoint_identification_algorithm,
		 "none",
		 NULL,
		 NULL,
		...
		{{"kafka_sasl_mechanism",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Alias for sasl.mechanisms: SASL mechanism to use for authentication. Supported: GSSAPI, PLAIN, SCRAM-SHA-256, SCRAM-SHA-512, OAUTHBEARER. NOTE: Despite the name only one mechanism must be configured. Type: string"),
		  gettext_noop("Default: GSSAPI, Valid values: GSSAPI")},
		 &kafka_sasl_mechanism,
		 "GSSAPI",
		 NULL,
		 NULL,
		...
		{{"kafka_sasl_username",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("SASL username for use with the PLAIN and SASL-SCRAM-.. mechanisms. Type: string"),
		  gettext_noop("Default: NULL")},
		 &kafka_sasl_username,
		 NULL,
		 NULL,
		...
		{{"kafka_sasl_password",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("SASL password for use with the PLAIN and SASL-SCRAM-.. mechanism. Type: string"),
		  gettext_noop("Default: NULL")},
		 &kafka_sasl_password,
		 NULL,
		 NULL,
		...
		{{"kafka_compression_codec",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("compression codec to use for compressing message sets. This is the default value for all topics, may be overridden by the topic configuration property compression.codec. Type: enum value"),
		  gettext_noop("Default: none, Valid values: none, gzip, snappy, lz4, zstd")},
		 &kafka_compression_codec,
		 "none",
		 NULL,
		 NULL,
		...
		{{"kafka_client_id",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("Client identifier. Type: string"),
		  gettext_noop("Default: rdkafka")},
		 &kafka_client_id,
		 "rdkafka",
		 NULL,
		 NULL,
		...
		{{"kafka_debug",
		  PGC_SIGHUP,
		  CUSTOM_OPTIONS,
		  gettext_noop("A comma-separated list of debug contexts to enable. Detailed Producer debugging: broker,topic,msg. Consumer: consumer,cgrp,topic,fetch. Type: CSV flags"),
		  gettext_noop("Default: NULL, Valid values: generic, broker, topic, metadata, feature, queue, msg, protocol, cgrp, security, fetch, interceptor, plugin, consumer, admin, eos, mock, assignor, conf, telemetry, all")},
		 &kafka_debug,
		 NULL,
		 NULL,
		 NULL,
